...
def login(usern, passw):
    ...
    if ("library",) in cursor:
        ...
        cnx = mysql.connector.connect(
            user=usern, passwd=passw, host="localhost", database="library"
        )
        cursor = cnx.cursor()
        ...
        if ("books",) in cursor and ("members",) in cursor:
            ...
            execSQL(
                cursor,
                "CREATE TABLE 'library'.'members' (  'MemberID' INT NOT NULL AUTO_INCREMENT, 'FirstName' VARCHAR(45) NULL,  'LastName' VARCHAR(45) NULL,  'DOJ' DATE NULL,  PRIMARY KEY ('MemberID'));",
            )
            execSQL(
                cursor,
                "CREATE TABLE 'books' (  'BookID' BIGINT(25) NOT NULL AUTO_INCREMENT,  'ISBN' INT NOT NULL DEFAULT 00000,'Title' VARCHAR(100) NOT NULL DEFAULT '--nobook--','LendedTo' INT NOT NULL DEFAULT -1,PRIMARY KEY ('BookID'));",
            )
            user = usern
            pas = passw
            return True
        ...
        execSQL(
            cursor,
            "CREATE TABLE 'members' ('MemberID' INT NOT NULL AUTO_INCREMENT, 'FirstName' VARCHAR(45) NULL, 'LastName' VARCHAR(45) NULL, 'DOJ' DATE NULL, PRIMARY KEY ('MemberID'));",
        )
        execSQL(
            cursor,
            "CREATE TABLE 'books' (  'BookID' BIGINT(25) NOT NULL AUTO_INCREMENT,  'ISBN' INT NOT NULL DEFAULT 00000,'Title' VARCHAR(100) NOT NULL DEFAULT '--nobook--','LendedTo' INT NOT NULL DEFAULT -1,PRIMARY KEY ('BookID'));",
...
def parseCommand(stat=str):
    cnx = mysql.connector.connect(
        user=user, passwd=pas, host="localhost", database="library"
    )
    cursor = cnx.cursor()
    if stat.lower().startswith("user"):
        if stat.lower().startswith("user add"):
            ...
            if "(" not in stat or ")" not in stat:
                ...
                execSQL(
                    cursor,
                    f"SELECT * FROM members WHERE CONCAT(FirstName, ' ', LastName) like '%{args}%'",
                )
                temp = []
                for x in cursor:
                    temptemp = []
        ...
        if stat.lower().startswith("book add"):
            ...
            if "(" not in stat or ")" not in stat or "," not in stat:
                ...
                if lendstatus != (-1,):
                    print(
                        "Book has already been lended to memberID #"
                        + str(lendstatus[0])
                    )
                else:
                    try:
                        execSQL(
                            cursor,
                            f"UPDATE books SET LendedTo = {args[1]} WHERE BookID = {args[0]};",
                        )
                        success()
                    except:
                        commandFailedNoRecords()
            ...
            if "(" not in stat or ")" not in stat:
                ...
                try:
                    execSQL(
                        cursor,
                        f"UPDATE books SET LendedTo = default WHERE BookID = {args};",
                    )
                    success()
                except:
                    commandFailedNoRecords()
            ...
            if "(" not in stat or ")" not in stat:
                ...
                execSQL(
                    cursor,
                    f"SELECT books.BookID, books.ISBN, books.Title, ifnull(members.MemberID, '-1') as LendeeID, ifnull(concat(members.FirstName, ' ', members.LastName), '--not lended--') as LendeeName FROM books LEFT JOIN members ON books.LendedTo = members.MemberID WHERE BookID = '{args}';",
                )
                temp = []
                for x in cursor:
                    temptemp = []
            ...
            if "(" not in stat or ")" not in stat:
                ...
                execSQL(
                    cursor,
                    f"SELECT books.BookID, books.ISBN, books.Title, ifnull(members.MemberID, '-1') as LendeeID, ifnull(concat(members.FirstName, ' ', members.LastName), '--not lended--') as LendeeName FROM books LEFT JOIN members ON books.LendedTo = members.MemberID WHERE ISBN = '{args}';",
                )
                temp = []
                for x in cursor:
                    temptemp = []
            ...
            if "(" not in stat or ")" not in stat:
                ...
                execSQL(
                    cursor,
                    f"SELECT books.BookID, books.ISBN, books.Title, ifnull(members.MemberID, '-1') as LendeeID, ifnull(concat(members.FirstName, ' ', members.LastName), '--not lended--') as LendeeName FROM books LEFT JOIN members ON books.LendedTo = members.MemberID WHERE Title like '%{args}%';",
                )
                temp = []
                for x in cursor:
                    temptemp = []
...
while True:
    ...
    if not loggedin:
        statement = input(
            "Enter username and password separated by a comma (username,password): \nLibMan> "
        )
